...
import { recoveryGoods, markRucksackMaterial } from "@/network/api/user";
...
function showReuslt(list) {
	goodList.value = list;
	window.$dev && console.log("goodList.value>>>>>>>>>>>>>>>>>>>>>>>>>>>list",goodList.value)
	let key = "choose"
	let value = true
	...
	for (let i = 0; i < goodList.value.length; i++) {
		...
		if( typeof goodList.value[i].sticker != 'undefined' && goodList.value[i].sticker )
		{
			sticker.value.push( {
				...
				goodsName : `“${ goodList.value[i].sticker.stickerLabel }”集字卡 | 不可炼化`,
				notAllowSell : true
			} )
		}
		...
		allPrice.value += Number(goodList.value[i].price)
	}
...
//转换材料
function markRucksackMaterials() {
	sticker.value = []
...
function onClickSell() {
	...
	if (choiceList.length > 0) {
		...
		for (let i = 0; i < choiceList.length; i++) {
			/// >>>>>>>>>>>>>>>>>>>>>
			params.rucksackGoodsIds.push(choiceList[i].id)
			/// <<<<<<<<<<<<<<<<<<<<<
		}
		markRucksackMaterial(params).then(res => {
			if (res.code == 0) {
...
<template>
	<div id="pc-box-result" :class="{ 'active': show }">
		<div class="mult-result-wrap" v-if="goodList.length > 1 || (goodList.length == 1 && goodList[0].sticker != null) || goodList[0].activityGoods != null">
			...
			<div class="prize-box-content">
				<div class="prize-box" :key="choosekey">
					...
					<div class="prize-item"
						...
						<div class="prize-item-back" :style="'background-image: url(' + getImageBg2(item.goodsLevel) + ');'">
					...
					<div v-for="(item, index) in goodList" :key="index" >
						<div  class="prize-item" v-if="item.sticker != null">
							<div class="prize-item-back">
								<div class="prize-item-pic-puzzle" >
									<img :src="item.sticker.stickerIconUrl" alt="">
								</div>
							</div>
							<div class="item-puzzle">
								<!-- <p class="price">{{ item.price }}</p> -->
								<p class="item-info-puzzle">{{ item.sticker.stickerLabel }}</p>
							</div>
						</div>
					</div>
					<div v-for="(item, index) in goodList" :key="index" >
						<div  class="prize-item" v-if="item.activityGoods != null">
							<div class="prize-item-back" :style="'background-image: url(' + getImageBg2(6) + ');'">
								<div class="prize-item-pic-puzzle" >
							...
							<div class="item-puzzle">
								<Price
									size="15"
									color="#7BDCA2"
									:currency="item.activityGoods.price"
								></Price>	
							</div>
						</div>
					</div>
			...
			<div class="opt-wrap">
				<!-- <div class="btn-all-recovery" @click="onClickSell">{{ '炼化' }}&nbsp; -->
					<!-- <img class="pc-price-coin" src="@/assets/pcimg/common/coin.png" alt=""/>{{ allPrice.toFixed(2) }}  -->
					<!-- <Price
						size="15"
						:currency="allPrice.toFixed(2)"
					></Price>
				</div>
				<div class="btn-recovery" @click="onClickSell">{{ '炼化选择的饰品' }}</div> -->
				<div class="btn-return" @click="onClose">{{ t( 'openbox.putInBag' ) }}</div>
			</div>
		</div>
		<div class="one-result-wrap dialog-wrap" v-if="goodList.length == 1 && goodList[0].sticker == null && goodList[0].activityGoods == null">
			<div class="close" @click="onClose">
				<img src="@/assets/pcimg/common/close.png" alt="" />
			</div>
