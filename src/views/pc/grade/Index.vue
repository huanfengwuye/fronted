	...
	<div id="pc-grade-wrap">
		...
		<div class="page-head">
			<div class="page-title">
				...
				{{ t( 'router.grade' ) }}
				<div class="total" v-if="hasLogin">当前盲盒点数 <label>{{ availablePoint }}</label></div>
			</div>
		...
		<div class="grade-list-wrap" v-else>
			...
			>
				<div v-if="item.boxItem" class="item-container">
					...
					<div class="grade-btn-refresh" v-if="gradeType != 0 && item.level != gradeType">{{ t( 'grade.refreshing' ) }}</div>
					<div class="grade-btn-already" v-else-if="item.level == gradeType">{{ t( 'grade.opened' ) }}</div>
					<div class="grade-btn-open" v-else-if="gradeType == 0 && Number(availablePoint) >= Number(item.needAmount)"
