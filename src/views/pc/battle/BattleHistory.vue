...
<template>
	<div id="pc-battleHistory">
		...
		<div>
			<van-list v-model:loading="allLoading" :finished="allFinished" :finished-text="t( 'common.noMore' )" @load="onLoad"
				:immediate-check="false">
				<div class="room_list">
					<div class="room_item" :class="{chou: item.winMode == 1}" v-for="(item, index) in recordList" :key="index" @click="openDetail(item.id)">
							...
							<div class="player-wrap">
								<div class="player-item" v-for="(playItem, index) in item.peoples " :key="playItem">
									<div class="player-icon" :class="{lose:item.battleUserDatas[index].winFlag == 2}">
										<Avatar :userModel='item.battleUserDatas[index]' :size="'pc-battle-header'" :hide-frame="true">
										</Avatar>
		...
		<div v-if="false" v-show="activeTab == 2" class="me-wrap">
			<div class="top-wrap">
				...
				<div class="top-item">
					...
					<div class="item-nodata" v-show="Number(statistics.dayMaxAmount) <= 0">{{ t( 'battle.historyNil' ) }}</div>
				</div>
			</div>
			<van-list v-model:loading="meLoading" :finished="meFinished" :finished-text="t( 'common.noMore' )" @load="onMeLoad"
				:immediate-check="false">
					...
					<div class="center">
						<div class="user">
							<div class="user_item" v-for="(son, index) in item.battleUserDatas" :key="index">
								...
								<Avatar :userModel="son" :size="'history'"></Avatar>
							</div>
						</div>
						<div class="price">
