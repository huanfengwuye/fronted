...
async function onClickAdd() {
	...
	params.winMode = battleInfo.value.winMode;
...
<template>
	<div id="pc-battleDetail" class="min-wrap-height">
		...
		<van-dialog
			v-model:show="showCopyAdd"
			show-cancel-button
			class="pc-join-dialog"
		>
			<div class="join-dialog-wrap">
			<div class="join-content-wrap">
				{{ t( 'battle.confirmCopy' ) }} &nbsp;
				<!-- <img src="@/assets/pcimg/common/coin.png"> <span>{{battleInfo.joinPrice}}</span> -->
				<Price fontWeight="700" color="#75dc9e" size="22" :currency="battleInfo.joinPrice"></Price>&nbsp;
			...
			<div class="join-dialog-bottom">
				...
				<div class="check-icon" @click="onCheckNotNotice">
					...
					v-if="notJoinNotice"
					class="checked-wrap"
					src="@/assets/pcimg/sign/checkbox__inner.png"
					alt=""
					/>
		...
		<div class="top-info" v-if="false">
			...
			<div class="top-rand-info" v-if="battleInfo.randValue">
				...
				<span class="info-verify" @click="onVerifyBattle" v-if="showVerifyBtn">{{ t( 'battle.verifyTitle' ) }}</span>
			</div>
			<p class="bat-type-wrap">{{ battleInfo.battleType == 1 ? t( 'battle.defaultBattleTitle' ) : t( 'battle.XBattleTitle' ) }}</p>
		</div>
		<Operation :boxList="boxList" :watchCount="battleInfo.watchUserCount" :start="start" :winMode = winMode
