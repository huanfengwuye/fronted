...
let searchTabData = ref({
	searchItems: [
		...
		{ icon: "&#xe61c;", text: '心跳不止', min: 5, max: 10 ,type:6},
...
function onClickHelp() {
	let data = {
		show: true,
		html: `<p>${ t( 'router.battle' ) }</p><br><p>${ t( 'battle.ruleText1' ) }</p><br><p>${ t( 'battle.ruleText2' ) }</p><br><p>${ t( 'battle.ruleText3' ) }</p><br><p>${ t( 'battle.ruleText4' ) }</p><br><p>${ t( 'battle.ruleText5' ) }</p><br><p>${ t( 'battle.ruleText6' ) }</p><br><p>${ t( 'battle.ruleText7' ) }</p>`
	}
	...
	<div id="pc-createbattle">
		...
		<div class="pc-battle-title">
			<div class="title-c">
				...
				<p>{{ t( 'battle.addBox' ) }}</p>
				<div class="question"  @click="onClickHelp"><img src="@/assets/pcimg/battle/question.png" alt="" /></div>
			</div>
			<div class="opts">
		...
		<div class="create-nav">
			<div class="nav-left">
				...
				<div class="model">
					<p>房间模式</p>
					<div class="modelType" :class="{active: checked }" @click="homeType">
						标准模式
					</div>
					<div class="modelType" :class="{active: !checked }" @click="homeType">
						非酋模式
				...
				<!-- <p v-if="checked">非酋模式</p>
				<p v-else>标准模式</p> -->
				<!-- <p>{{winMode}}</p> -->
				<!-- <van-switch class="custom-switch" v-model="checked" active-color="#5AC8CE" inactive-color="#414461" /> -->
			</div>
			<div class="nav-right">
				{{ t( 'battle.priceTotal' ) }}&nbsp;&nbsp;
				<!-- <span>
					<img class="pc-price-coin" src="@/assets/pcimg/common/coin.png" alt=""/>{{ totalPrice.toFixed(2) }}
				</span> -->
				<Price
					size="14"
					color="#75DC9E"
		...
		<div class="box-wrap">
			<div class="box-item" v-for="(item, index) in already" :key="index">
				...
				<div class="price">
					<!-- <img class="pc-price-coin" src="@/assets/pcimg/common/coin.png" alt=""/>{{ (Number(item.price) * Number(item.amount)).toFixed(2) }} -->
					<Price
						size="14"
						color="#75DC9E"
						fontWeight="700"
						:currency="(Number(item.price) * Number(item.amount)).toFixed(2)"
					></Price>
				</div>
				...
				<div class="create-body">
					...
					<div class="goods-list">
						<div class="goods-item" v-for="(item, index) in listBox" :key="index" :class="{ active: item.amount > 0 }"
							...
							<div class="show-box-detail" @click.stop="getBoxGoods( item )"><img src="@/assets/pcimg/battle/search.png"></div>

							<div class="num" v-show="item.amount > 0">×{{ item.amount }}</div>
							<div class="reduce" v-show="item.amount > 0" @click.stop="boxCatAmount( item )"><img src="@/assets/pcimg/battle/minus.png"></div>
...
// 	.van-switch__node
// 	{
// 		transform: translate( calc( var(--van-switch-width) - var(--van-switch-node-size ) - 10px ) );
// 	}
// }
...
#pc-createbattle {
	...
	.switch-container {
	display: flex;
	align-items: center; /* 使用 Flex 布局，垂直居中对齐 */
	justify-content: center; /* 使用 Flex 布局，水平居中对齐 */
	height: 100px; /* 设置容器高度，可以根据实际情况调整 */
	}

	/* 可以根据需要调整 .van-switch__node 的样式 */
	.switch-container .van-switch__node {
		top: 5px !important;
	}
