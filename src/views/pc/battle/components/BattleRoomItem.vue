...
onMounted(() => {
	...
	window.$dev && console.log( "props.userData",props.userData )
...
function openPrize() {
	...
	window.$dev && console.log("winGoodsItems.value",winGoodsItems.value )
	...
	>
		...
		>
				...
				<div class="state" v-show="waiting && clientRoomStatus == 1">
					<div class="wait">
						<div v-show="!userData.noBody" class="wait-item ready">
							<img
								src="@/assets/pcimg/battle/battle-user-reday-bg.png"
								alt=""
							/>
							<!-- 准备 -->
						</div>
						<div v-show="userData.noBody" class="wait-item">
							<img
								src="@/assets/pcimg/battle/battle-user-waiting-bg.png"
								alt=""
							/>
							<!-- 等待玩家 -->
						...
						<div :class="[userData.winFlag != 1 ? 'loser' : 'winner']">
							...
							>
								<div class="result-price">
									...
									<Icon name="goldj" color="#FFB97A" size="23" ></Icon>&nbsp;
									<count-to
										:startVal="usedPrice"
										:endVal="userPrice || 0"
										:duration="800"
			...
			<div class="wait-join-wrap" v-if="userData.noBody">
				<button
					:class="['join', selfJoinStatus ? 'disabled' : '']"
					:disabled="selfJoinStatus"
					@click="joinRoom"
				>
					{{ t( 'battle.join' ) }}
				</button>
...
#pc-room-item {
	...
	.room-main-wrap {
		...
		.room-center {
			...
			&.waiting {
				background: #0d0e1a url(@/assets/pcimg/battle/battle-user-body-bg1.png)
					no-repeat;
				background-size: cover;
			}

			&.nil {
				background: #0d0e1a url(@/assets/pcimg/battle/battle-user-body-bg2.png)
					no-repeat;
				background-size: cover;
			}
			...
			.state {
				...
				.result {
					...
					.winner {
						...
						left: 0;
						background: #0d0e1a
							url(@/assets/pcimg/battle/battle-user-body-bg1.png) no-repeat;
						background-size: 100% 100%;
					...
					.loser {
						background: #0d0e1a
							url(@/assets/pcimg/battle/battle-user-body-bg2.png) no-repeat;
						background-size: 100% 100%;
