			...
			<div class="room_list">
				<transition-group name="animation">
					<div class="room_item" v-for="(item) in listRoom" :key="item.id"
						:class="[{ 'over': item.status > 2 }, { 'look': item.status == 2 }, { 'anchor': isAnchorRoom(item) && item.winMode == 0 }, { 'ordinary-chou': !isAnchorRoom(item) && item.winMode == 1 }, { 'anchor-chou': isAnchorRoom(item) && item.winMode == 1 }]" @click="enterRoom(item)">
						<div class="round" :class="{'active': item.winMode == 1}">{{ item.round }}</div>
						<div class="boxs">
						...
						<!-- {{item.joinPrice }} -->
						<Price :value="item.joinPrice" fontWeight="700" before-size="20px" after-size="16px" color="#75DC9E"></Price>
						</div>
						<div class="player">
