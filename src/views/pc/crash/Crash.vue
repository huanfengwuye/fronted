		...
		<van-overlay :show="showStore" z-index="300" style="background: rgba( 0, 0, 0, .7 )">
			<div class="pool-goods-container">
				...
				<div class="pool-goods-body">
					<van-list
						v-model:loading="poolGoodsLoading"
					>
						<div class="pool-goods-item-container"
							v-for="( item, index ) in poolGoods"
							:key="index"
				...
				<div class="crash-main">
					<div class="crash-history">
						<div class="history-item"
							:class="[ {'green' : item > 2 },{ 'gold' : item > 5 } ]"
							v-for="( item, index ) in boomPointRecord" :key="index"
						>{{ item }}</div>
					</div>
					...
					<div class="bet-container">
						...
						<!-- <div class="bet-wrap" v-show="false"> -->
							...
							<div class="next-round-at">
								<count-to
									:startVal="betOvertime"
									:endVal="0.00"
									:duration="betOvertime * 1000"
									:decimals="2"
									:useEasing="false"
									ref="countToRef"
								></count-to>
						...
						<p :class="{active:false}" v-show="gameStatus == 'GAME_PLAY'">x{{ Number( curBoomPoint ).toFixed( 2 ) }}</p>
					...
					<div class="crash-footer">
						<div class="btn" :class="{ active : !isJoin && gameStatus !== 'GAME_START' }">
							<button @click="action">{{ isJoin ? t( 'crash.runaway' ) : t( 'battle.join' ) }} <label><img src="@/assets/pcimg/crash/gold.png"/> {{ myCurBetMoney.toFixed( 2 ) }}</label></button>
						</div>
						...
						<div class="my-item-container">
							<div class="presets-container">
								...
								<div class="presets-wrap">
									...
									<div class="presets">
										...
										<button class="clear" v-show="targetGoods" @click="clearTargetGoods">{{ t( 'common.clear' ) }}</button>
							...
							<div class="bet-item-run" v-show="true">
								...
								<div class="bet-item-body">
									<img src="@/assets/pcimg/crash/gold.png"/>
									<input :placeholder="t( 'crash.multiple' )" v-model="myBetMoney" @blur="minBetMoney" @change="onChangeMultiple">
							...
							<div class="bet-items-container">
								...
								>
									...
									<div class="item-container">
										<div class="item"
											v-for="( item, index ) in myBagItems"
											:key="index"
										>
											<div class="checkbox">
												<input type="checkbox">
				...
				<div class="crash-sidebar">
					<div :class="[ 'user-container', !isBoom ? 'wait' : 'lose' ]">
						<div class="u-title">
							...
							<div class="price-total"><img src="@/assets/pcimg/crash/golds.png">{{ Number( moneyPool ).toFixed( 2 ) }}</div>
						</div>
						<transition-group name="list">
							<div class="user-item" v-for="( item, index ) in betUsers" :key="index">
								...
								<div class="user-item-container">
									...
									<div class="times-container" v-show="!isBoom">{{ Number( item.betItem.setMultiple ) <= 1 ? `LIVE` : `x${ item.betItem.setMultiple }` }}</div>
		...
		.crash-header
		{
			...
			.crash-header-item
			{
				...
				&.wanted{
					display: flex;
					align-items: center; /* 垂直居中 */
					background-image: url("@/assets/pcimg/crash/wanted.png"); /* 替换为你的图片 URL no-repeat*/
				...
				&.token{
					display: flex;
					align-items: center; /* 垂直居中 */
					background-image: url("@/assets/pcimg/crash/token.png"); /* 替换为你的图片 URL no-repeat*/
					background-size: contain;
					background-repeat: no-repeat;
					opacity: .4;//透明度设置
					.token-text{
				...
				&.shooting{
					display: flex;
					align-items: center; /* 垂直居中 */
					background-image: url("@/assets/pcimg/crash/shooting.png"); /* 替换为你的图片 URL no-repeat*/
					background-size: contain;
					background-repeat: no-repeat;
					opacity: .4;//透明度设置
					.shooting-text{
