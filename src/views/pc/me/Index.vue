...
import EAvatar from './personal/Avatar.vue';
...
const hasReal = computed( () => store.state.userInfoExt.idNumber != '' && store.state.userInfoExt.realName != '' )//ref(false);
...
function hideMiddleDigits(phoneNumber) {
    // 检查输入的手机号是否合法，长度必须为11位
    if (phoneNumber.length !== 11) {
        return "Invalid phone number";
    }

    // 将手机号的中间四位数字替换为*
    const hiddenNumber = phoneNumber.substr(0, 3) + "****" + phoneNumber.substr(7);
...
<template>
	<div id="pc-me">
		<div class="top-info" v-if="hasLogin">
			<div class="header-info">
				<Avatar :userModel='userInfoBase' :size="'pc-me-header'" @click="onClickEditAvatar()"></Avatar>
			...
			<div class="top-info-wrap">
				...
				<div class="info-item">
					...
					<div class="item-c">{{ hideMiddleDigits( userInfoBase.mobile ) }}</div>
			...
			<div class="top-opt-wrap">
				<div class="top-opt-item" @click="openEditReal">
					<svg class="icon-switch">
						<use xlink:href="@/assets/fonts/icon.svg#pencil" ng-href="@/assets/fonts/icon.svg#pencil"></use>
					</svg>
		...
		<div class="top-tabs" v-if="hasLogin">
			<div class="top-tab-item" :class="{ active: route.name == 'p_bag' }" @click="jumpRoute('/p/me/bag')">
				<svg class="icon-tab">
					<use xlink:href="@/assets/fonts/icon.svg#user" ng-href="@/assets/fonts/icon.svg#user"></use>
				</svg>
				{{ t( 'users.inStock' ) }}
			</div>
			...
			<div class="top-tab-item" :class="{ active: route.name == 'p_information' }"
				...
				<div v-if="noReadMessages" class="hint"></div>
				{{ t( 'router.message' ) }}
			</div>
			...
			<div class="tab-vip" :class="{ active: route.name == 'p_vip' }" @click="jumpRoute('/p/me/vip')">
				{{ t( 'users.upgradeVip' ) }}
				<svg class="icon-tab">
					<use xlink:href="@/assets/fonts/icon.svg#premium" ng-href="@/assets/fonts/icon.svg#premium"></use>
				</svg>
		...
		<van-overlay :show="diagEditPW" style="background-color: rgba(0,0,0,.7);" :z-index="200" :lock-scroll="false">
			<div class="wrapper">
				<div class="dialog-c-wrap">
					...
					<div class="input-wrap">
						<input type="text" v-model="inputPWContent" maxlength="16" :placeholder="t( 'users.enterPasswordTip' )">
		...
		<EAvatar :showState="showAvatar" @close="onCloseAvatar" ref="avaterRef"></EAvatar>
	</div>
</template>
