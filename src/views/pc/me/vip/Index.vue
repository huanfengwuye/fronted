...
const vipAddList = ["-","-","0.5","1","1.5","2","2.5","3","4","5","6","7"]
...
async function getVipLevelRecord() {
	...
	if (res.code === 0) {
		...
		// console.log(">>>>>>>>>>>>>>",vipRate.value = level.value/(items.length-1)*100)
	}
}
...
<template>
	<div id="pc-vip">
		<div class="vip-content">
			<div class="vip-title">{{ t( 'vip.title' ) }}
				<!-- <img src="@/assets/pcimg/vip/help.png" alt="" /> -->
			</div>
			<div class="list-wrap">
				<div class="progress-wrap">
					<van-progress :percentage="vipRate" pivot-text="" track-color="#12463F"
					color="#79ECDD"/>
				</div>
				<div class="vip-list-wrap">
					<div class="vip-item" v-for="item in levelList" :key="item.level">
						<div class="item-first">{{ item.needAmount }}</div>
						<div v-if="item.givePercent != '0.00'" class="item-second">{{ item.givePercent }}%</div>
						<div v-else class="item-second">-</div>
						<div class="item-addition">{{ item.rewardAmount }} </div>
						<div v-if="item.status == 1">
							<p v-if="item.level == 0" class="vip0-p"> - </p>
							<p v-else class="status" @click="getWelfare(item)">{{ t( 'redpack.receive' ) }}</p>
						</div>
