...
// // 当搜索价格数据时，重置过滤价格项目并重新加载商品列表
// function onSearchPriceData() {
//     curFliterPriceItem.value = {}; // 重置价格筛选条件为空对象
//     pager.value.pageIndex = 1; // 重置分页索引为第一页
//     finished.value = false; // 设置未完成标志为false
//     getGoodsList(); // 获取商品列表
// }

// 当搜索数据时，重置分页索引并重新加载商品列表
...
// 当点击排序按钮时，设置房间排序类型并重新加载商品列表
function onClickSort(sortType) {
    roomSort.value = sortType; // 设置房间排序类型
    filterSelect.value.roomSort = sortType; // 更新筛选条件中的房间排序类型
    pager.value.pageIndex = 1; // 重置分页索引为第一页
    finished.value = false; // 设置未完成标志为false
    getGoodsList(); // 获取商品列表
}

// 当加载页面时，如果商品正在加载中，则不执行任何操作，否则增加分页索引并重新加载商品列表
function onLoad() {
	window.$dev && console.log( 'load more' )
    if (goodsLoading.value) return; // 如果商品正在加载中，则不执行以下操作
    pager.value.pageIndex += 1; // 增加分页索引
    getGoodsList(); // 获取商品列表
}
		...
		<div class="lucky-top">
			<div class="type-items">
				<div
					...
					v-for="(item, index) in typeItemsArray"
					:key="index"
			...
			<div class="goods-header">
				<div class="header-left">
					<svg class="icon-cubes">
						<use
							xlink:href="@/assets/fonts/icon.svg#cubes"
							ng-href="@/assets/fonts/icon.svg#cubes"
						></use>
					</svg>
			...
			<div class="fliter-wrap">
				<div class="fliter-left">
					<div class="top-sort-item">
						<div
							class="top-sort-up"
							:class="{ active: roomSort != 2 }"
							@click="onClickSort(1)"
						>
							<svg class="icon-arrow">
								<use
									xlink:href="@/assets/fonts/icon2.svg#arrow-up"
									ng-href="@/assets/fonts/icon2.svg#arrow-up"
								></use>
							</svg>
						</div>
						<div
						...
						>
							<svg class="icon-arrow">
								<use
									xlink:href="@/assets/fonts/icon2.svg#arrow-down"
									ng-href="@/assets/fonts/icon2.svg#arrow-down"
								></use>
							</svg>
						</div>
					</div>
...
#pc-luckybox {
	...
	.goods-wrap {
		.goods-header {
			...
			height: 88px;
			padding-left: 30px;
			background: url(@/assets/pcimg/lucky/sec_title.svg) left center/contain
				no-repeat #1b1d35;
			box-sizing: border-box;
