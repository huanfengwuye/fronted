...
onMounted(() => {
	if (route.query.type == "nickName") {
		editObj.value = {
			...
			placeholder: t('sign.enterNickname'),
			maxLength: 10,
			minLength: 3,
...
function downImg(url) {
	...
	a.setAttribute("download", "mskins_auth");
...
function confirmReal() {
	...
	realName(params).then((res) => {
		// if (res.code == 0) {
		// 	console.log( res )
		// 	NotifyS( t('users.realnameRewardTip', { num : res.data.rewardAmount } ) );
		// 	store.dispatch("getUserInfo");
		// 	router.go(-1);
		// }
		...
		if (res.code == 0) {
			authUrl = "alipays://platformapi/startapp?appId=20000067&url="+encodeURIComponent(res.data["verifyUrl"]);
