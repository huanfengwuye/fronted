	...
	setTimeout(() => {
		...
		if (
			...
			sticker.value.unshift({
				goodsLevel: 6,
				goodsName: `“${
					props.userData.openBoxDatas[props.roomRound].sticker.stickerLabel
				}”集字卡 | 不可炼化`,
				price: "0.00",
				iconUrl:
...
<template>
	<div
		id="room-item"
		:class="[{ 'two-pep': peoples == 2 }, { 'three-pep': peoples == 3 }]"
	>
		<div class="room-main-wrap">
			<div class="user-info-wrap" v-if="clientRoomStatus == 3">
				<div class="user-header-wrap" :class="{ lost: userData.winFlag == 2 }">
					...
					<p
						style="color: #4af1cc"
						v-if="userData.winFlag == 1 || userData.winFlag == 3"
						class="win-text-wrap"
					>
						{{ t('battle.winner') }}
					</p>
					...
					<div class="wingot"
						v-if="Number( userData.winAmount ) > 0 && roomStatus == 3"
					>+ {{ userData.winAmount }}</div>
				</div>
			...
			<div class="user-info-wrap" v-show="clientRoomStatus == 2">
				...
				>
					<BattleAction
						v-show="!waiting && clientRoomStatus == 2"
						:order="userData.orderList"
						:boxList="boxList"
						ref="battleActRef"
						:openBoxItems="userData.openBoxDatas"
						:roomRound="roomRound"
						:start="props.start"
			...
			<div
				...
				v-if="!userData.noBody && clientRoomStatus == 1"
			>
				<div class="user-header-wrap" :class="{active:winMode == 1}">
			...
			>
				<div class="join-wrap">
					...
					<img
						class="circular"
						src="@/assets/romimg/battle/room/two/circular.png"
					/>
		...
		<div
			...
			:class="{ win: clientRoomStatus == 3 && userData.winFlag != 2 }"
		>
			<div class="goods-list" v-show="clientRoomStatus != 1">
				<div class="goods-item" v-for="(item, index) in sticker" :key="index">
					...
					<div class="price">
						<!-- ${{ item.price }} -->
						<Price size="13" fontWeight="700" color="#7EF2AD" :currency="item.price"></Price>
					</div>
					<p class="name hide">
				...
				<transition-group name="animation">
					<div
						class="goods-item"
						v-for="(item, index) in winGoodsItems"
						:key="index"
...
#room-item {
	.room-main-wrap {
		...
		.wait-join-wrap {
			...
			.join {
				background: url("@/assets/romimg/battle/detail/wait_bg.png") no-repeat
					center;
				background-size: 100% 100%;
				position: relative;
				width: 60px;
		...
		.room-main-wrap {
			...
			.user-info-wrap {
				...
				.user-header-wrap {
					...
					&.active{
						border-top: 0px solid #75dc9e;
						background: url("@/assets/romimg/battle/detail/h5-chou.png")center/cover no-repeat;
					...
					.lost {
						width: 204px;
						height: 110px;
						background: url(@/assets/romimg/battle/detail/lost_bg1.png)
							no-repeat;
						background-size: contain;
						line-height: 110px;
						text-align: center;
