...
let searchTabData = ref({
	searchItems: [
		...
		{ icon: "", text: '神龙召唤', min: 5, max: 10 ,type:7},
		...
		{ icon: "", type: 6, text: '心跳不止', min: 0, max: 100 },
	],
	searchTabIndex: 0,
	searchFilter: "",
});
...
async function onClickCreate() {
	...
	if (res.code === 0) {
		NotifyS(t( 'battle.createSuccess' ));
...
function deletAll()
{
	alreadyList.value = []
	already.value = []
	for (let i = 0; i < listBox.value.length; i++) {
		listBox.value[i].amount = 0; // 将每个项目的 amount 属性设置为 0
	}
	// console.log("listBox的值",listBox.value)
	roundcalculation(); // 更新计算
}
...
// // 使用 watch 监听 checked 变化
// watch(checked, (newValue, oldValue) => {
//   // 在这里更新 winMode 的值，使用 checked.value 获取实际值
//   winMode.value = checked.value ? 1 : 0;
// });
...
<template>
	<div id="createbattle">
		<div v-if="false" class="bat_type_wrap">
			<div class="cb_tab">
				<div
					@click="onClickBattleType(1)"
					class="tab_item"
					:class="{ active: battleType == 1 }"
				>
					{{ t('battle.defaultBattle') }}
				</div>
				<div
					@click="onClickBattleType(2)"
					class="tab_item"
					:class="{ active: battleType == 2 }"
				>
					{{ t('battle.XBattle') }}
				</div>
			</div>
		...
		<div class="battle-header">
			...
			<div class="pep-wrap">
				...
				<div
					class="pep-item"
					@click="peopleCount = 3"
					:class="{ active: peopleCount == 3 }"
				>
					{{ t('battle.roomFor3') }}
				</div>
			</div>
			<div class="unlucky" @click="homeType" :class="{'active': checked}">
				<img src="@/assets/romimg/battle/create/type.png" alt="">
				<p v-if="!checked">标准模式</p>
				<p v-else>非酋模式</p>
			</div>
		</div>
		...
		<div class="search-container">
			...
			>
				<swiper-slide
					class="search-item"
					:class="{ active: index == searchTabData.searchTabIndex }"
					v-for="(item, index) in searchTabData.searchItems"
					@click="onClickSearchTabIndex(index)"
					:key="index"
				>
					<span v-if="item.icon" class="iconfont pc-icon-16">&#xe61c;</span
					>{{ item.text }}
				</swiper-slide>
			</swiper>
		...
		<ShowDialog
			@cancel="createTips.show = false"
			:btnType="-1"
			:title=" t('battle.create')  "
			:show="createTips.show"
		>
			<div class="show-dialog-content-wrap">
				<div class="desc-wrap">{{ t('battle.createTipText') }} </div>
				<div class="price-wrap">
					<Price :fontsize="24" :currency="totalPrice.toFixed(2)" />
				</div>
				<div class="desc-wrap">{{ t('battle.createConfirmText') }} </div>
				<div
					class="check-desc-wrap"
					@click="createTips.checked = !createTips.checked"
				>
					<img
						v-if="createTips.checked"
						src="@/assets/romimg/battle/create/checked.png"
						alt=""
					/>
					<img
						v-else
						src="@/assets/romimg/battle/create/unchecked.png"
						alt=""
					/>
					{{ t('battle.resultTip') }} 
				</div>
		...
		<div class="goods-list" :class="{ active: alreadyList.length > 0 }">
			...
			>
				...
				<div class="price">
					<!-- <Price :fontsize="12" :currency="item.price"></Price> -->
					{{ item.price }}
		...
		<div class="bottom-wrap" :class="{ active: alreadyList.length > 0 }">
			<div class="spread" v-if="alreadyList.length > 0">
				...
				<div class="spread_item">
					{{ t('battle.priceTotal') }}：
					<Price :fontsize="12" color="#7EF2AD" :currency="totalPrice.toFixed(2)"></Price>
				</div>
				<div class="deletItem" @click="deletAll">
					<p>清除</p>
					<Icon class="icon" name="delete" :size="24"></Icon>
				</div>
			</div>
			...
			<div class="create-btn">
				<!-- <div class="unlucky-guy" :class="{'active': checked}" @click="homeType">
				</div> -->
				<p v-if="alreadyList.length > 0" @click="onClickCreatePre">{{ t('router.createBattle') }}</p>
				<p v-else class="">{{ t('router.createBattle') }}</p>
			</div>
		</div>
	</div>
	...
	.bottom-wrap {
		...
		.spread {
			...
			.deletItem{
				display: flex;
				align-items: center;
				margin-left: auto; /* 将所有剩余空间放在元素左侧，实现向右对齐 */
		...
		.create-btn {
			...
			.unlucky-guy{
				...
				border-radius: 8px;
				background: url(@/assets/romimg/battle/standard.png);
				background-size: cover; /* 将背景图等比例缩放，保持完全填充容器 */
				background-position: center; /* 将背景图放置在容器中央 */
				background-repeat: no-repeat; /* 禁止背景图重复 */
				&.active{
					background: url(@/assets/romimg/battle/clown.png);
					background-size: cover; /* 将背景图等比例缩放，保持完全填充容器 */
					background-position: center; /* 将背景图放置在容器中央 */
					background-repeat: no-repeat; /* 禁止背景图重复 */
				}
