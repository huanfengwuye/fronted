...
const activeTab = ref(2); //1-全部 2-我的
...
function openDetail(id) {
	router.push({ path: "/m/battleDetail", query: { id: id } });
}
		...
		<div v-if="activeTab == 2" class="me-wrap">
			...
			>
				<div
					...
					@click="openDetail(item.id)"
				>
					<div class="center" :class="{ fail: !judgeSelfWin(item) && item.winMode == 0, active: judgeSelfWin(item)  && item.winMode == 0, victoryChou: !judgeSelfWin(item) && item.winMode == 1, fallChou: judgeSelfWin(item) &&  item.winMode == 1  }">
						<div class="price" :class="{ fail: judgeSelfWin(item) && item.winMode == 0, victory: !judgeSelfWin(item) && item.winMode == 0, chou: item.winMode == 1 }">
							<Price
								fontcolor="white"
								:currency="item.maxWinPrice.toFixed(2)"
								size="13"
							></Price>
							<div class="round">
								<span class="number_span">{{ item.round }}</span
...
#battleHistory {
	...
	.history_item {
		...
		.center {
			...
			&.fallChou {
				background: url(@/assets/romimg/battle/history/fallChou.png)no-repeat;
				background-position: center;
				background-size: contain; /* 或者使用 contain，具体根据需求选择 */
			}

			&.victoryChou {
				background: url(@/assets/romimg/battle/history/victoryChou.png)no-repeat;
				background-position: center;
				background-size: contain; /* 或者使用 contain，具体根据需求选择 */
			}
			// line-height: 90px;
			...
			.price {
				...
				// &.chou {
				// 	background: url(@/assets/romimg/battle/history/historyChou.png)no-repeat;
				// 	background-position: center;
				// 	background-size: contain; /* 或者使用 contain，具体根据需求选择 */
				// }
				height: 80px;
