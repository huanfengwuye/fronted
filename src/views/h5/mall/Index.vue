...
async function getList() {
	...
	if (ammoArea.value.max != 0) {
		params.minPoint = ammoArea.value.min + "";
		params.maxPoint = ammoArea.value.max + "";
	}
...
async function onSureExchange() {
	...
	if (res.code == 0) {
		...
		if (rucksackGoodsId != 0) {
			NotifyS( t( 'mall.exchangeSuccess' ) );
			...
			<div class="sbtab_nav_wrap">
				...
				>
					...
					<img
						...
						class="pic"
						src="@/assets/romimg/mall/mall_t9.png"
						alt=""
					/>
					...
					<div class="drop-down-wrap">
						...
						<div class="drop-down" :class="{ active: qualityActive }">
							<div class="down-text hide" @click="onOtherDown">
								{{ qualityOptItem.name }}
								<p v-show="qualityTextShow">{{ t('common.category') }}</p>
								<img src="@/assets/romimg/common/arrow_down.png" alt="" />
							</div>
								...
								>
									<div class="price active">
										<div class="ammunition">
											<!-- <Price size="12" fontWeight="700" color="#7EF2AD" :currency="item.price"></Price> -->
											<Icon name="diamond" :size="16"></Icon>&nbsp;{{ item.point }}
										</div>
									</div>
		...
		<!-- <div class="popup" :class="{ active: DetailScreenShow }">
			<div class="popup-wrap">
				...
				<div class="popup-content">
					<div class="help_detail_info">
						<p>
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;尊敬的用户您好，如您的商城可兑换金额小于当前金币余额，说明近期充值金额未形成足额开箱流水，为防止刷子团队利用规则漏洞干扰正常网站用户、以及预防不法分子洗钱，请您在充值后按照正常流程进行开箱、炼化等操作，商城可兑换金额会产生相应累计。
						</p>
						<p>
							*注：普通盲盒、幸运饰品、决战沙场里获取的饰品，炼化后产生流水。其余福利性质盲盒、Roll房、或活动赠送饰品的炼化不计入流水。
						</p>
					</div>
				</div>
			</div>
				...
				<div class="popup-content">
					<div class="popup-main">
						<div class="result-totle-price">
							<Icon name="diamond" :size="16"></Icon>&nbsp;{{ exchangeSelItem.point }}
							<!-- <Price :currency="exchangeSelItem.price"></Price> -->
						</div>
						<div
							class="one-result"
							:style="
