	...
	if (res.code == 0) {
		...
		for (let i = 0; i < card.value.length; i++) {
			...
			for (let j = 0; j < payTypeIconWithNames.length; j++) {
				...
				// 找到匹配的 name，可以执行相关操作
				window.$dev && console.log(`找到匹配项：${item[0]}`);
...
const shanguo = ref(null);

function jumpLink() {
	window.location.href = shanguo.value;
}
	...
	if (res.code == 0) {
		...
		if (_isMobile()) {
			...
			if (curRechageType.value.payMode == 1) {
				...
				// if(urlTpe.value == 0 && curRechageType.value.payType == 13){
				// 	window.location.href = shanguo.value;
...
function downImg(url) {
	...
	a.setAttribute("download", "mskins");
...
function getRechargeTypeName(item) {
	...
	if (item.payType == payTypeArray.bank) {
		...
		//微信
		name = "Card Pay" ;
	} else if (item.payType == payTypeArray.shande) {
		//微信
		name = "杉德" ;
	}
	return name;
...
<template>
	<div id="recharge" class="min-wrap-height">
		<div class="container">
			...
			<div class="level-wrap">
				...
				<div class="score">
					{{ t('vip.stillFromNextLevel')}} 
					<span v-if="nextVipAmount >= 0 ">{{ nextVipAmount }}</span>
					<span v-else>{{ 0 }}</span>
				</div>
			</div>
			<div class="recharge-wrap">
				<div class="recharge-type" v-if="!isApplyPay">
					...
					<div class="recharge-comment">
						<!-- <span style="color: #e9c529">充值加成将于2024年3月1日更新为vip加成</span> -->
					</div>
					<div class="recharge-type-list">
						...
						>
							...
							>
								...
								<div class="cards-pay" v-if="item.payType == payTypeArray.chargeCard  && curRechageIndex == index" >
									<div
					...
					<div class="recharge-content-wrap">
						...
						<div class="recharge-list-wrap" v-if="curRechageType.payType != 11">
							<div
								...
								:class="{ active: index == curMoneyIndex }"
							>
								<!-- <div> -->
									<Price
						...
						<div class="recharge-tip">
							<span>{{ t('recharge.confirmText')}}</span>
						</div>
					</div>
				</div>
				...
				<div class="recharge-wrap" v-if="false">
					...
					<div class="recharge-content-wrap">
						<div class="recharge-list-wrap">
							<div
								class="recharge-item"
								v-for="(item, index) in applyProducts"
								:key="index"
								@click="onClickMoneyItem(item, index)"
								:class="{ active: index == curMoneyIndex }"
							>
						...
						<div class="recharge-tip">
							<span>* 充值加成将于2024年3月1日更新为vip加成</span>
							<span>{{ t('recharge.confirmText')}}</span>
						</div>
					</div>
				</div>
			...
			<van-overlay :show="qrCodeShow && curRechageType.payType != 13">
				<div class="pop-code">
					<div class="pop-code-bg">
						<div class="pop-code-title">
							<!-- <img
								src="@/assets/romimg/recharge/viBlan.png" alt=""
							/>Bank QR -->
							<span class="icon iconfont close" @click="onQRCodeSure"
								>&#xe637;</span
							>
						</div>
			...
			<van-overlay :show="qrCodeShow && curRechageType.payType == 13 && urlTpe != 0">
				<div class="pop-code">
					<div class="pop-code-bg">
						<div class="pop-code-title">
							<!-- <img
						...
						<div class="qrCode_box">
							<div class="qrCode-warp" v-if=" urlTpe == 1 ">
								<img src="@/assets/romimg/recharge/hint.png" alt="">
								<p>您尚未开户，请前往开户</p>
								<span>后续充值将使用账户余额，请开户后进行储值操作。</span>
							</div>
							<div class="qrCode-warp" v-if=" urlTpe == 2 ">
								<img src="@/assets/romimg/recharge/hint.png" alt="">
								<p>您的账号余额不足，请前往充值</p>
								<span>后续充值将使用账户余额，请开户后进行储值操作。</span>
							</div>
						...
						<div class="qrCode_bottom">
							<div class="qrCord-but" @click="jumpLink">
								<p v-if=" urlTpe == 1 ">前往开户</p>
								<p v-if=" urlTpe == 2 ">前往充值</p>
							</div>
						</div>
					</div>
				</div>
		...
		<div class="fixed-bottom">
			<div class="recharge-money-data">
				...
				<div class="recharge-money-item arrival_amount">
					...
					<span>
						<Price
							:currency="(Number(curMoneyItem.amount) + Number(curMoneyItem.giveAmount)) * (1-cardRate).toFixed(2)"
						></Price>
					</span>
				</div>
			</div>
		...
		<div
			class="showPIC"
			v-show="showPic && curRechageType.payType == payTypeArray.taobao"
			@click="showPic = false"
		>
		...
		<div
			class="showPIC"
			v-show="showPic && curRechageType.payType == payTypeArray.km"
			@click="showPic = false"
		>
	...
	<ShowDialog
		:btn-type="1"
		:cancel-show="showDialog"
		:show="showDialog"
		@ok="userOkNeed"
		@cancel="hideUserNeed"
	>
		<div class="dialog-x-content">
			<div class="dialog-row-wrap">{{ t('redpack.acceptAgreementTitle')}}</div>
			<!--   :class="{ unchecked: !item.selected }" -->
			<div
			class="dialog-row-wrap"
