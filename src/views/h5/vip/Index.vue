	...
	if (item.level) {
		...
		getVipReward(params).then((res) => {
			if (res.code == 0) {
				...
				NotifyS(t('vip.receiveReward',{amount : res.data.amount}));
				getVipLevelRecord();
			}
		});
	}
...
async function getFrame() {
	...
	if (res.code == 0) {
		let items = res.data.items || [];
		// window.$dev && console.log("items", frameList.value);
...
<template>
	<div id="vip">
		<TopTitleBack :title="t('router.vip')"></TopTitleBack>
				...
				>
					<div class="swiper-item">
						...
						>
							...
							<img
								class="frame-img"
								v-if="index > 0 && frameList[index - 1] && frameList[index - 1].avaterFrame"
								:src="frameList[index - 1].avaterFrame"
								alt=""
							/>
							<img
								class="frame-img"
								v-else-if="index > 0 && frameList[index - 2] && frameList[index - 2].avaterFrame"
		...
		<div class="vip-table-wrap" v-if="tabsData.index == 0">
			<div class="vip-table-item-list">
				...
				>
					...
					<div class="red-reward">{{ item.rewardAmount }}</div>
