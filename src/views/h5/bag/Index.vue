...
<template>
	<div id="bag" class="min-wrap-height" @click.stop="onBaseClick">
				...
				>
					<div class="bag-1">
						...
						<div class="div-sum">
							...
							<Price size="12" color="#FFFFFF" font-weight="700" :currency="totalPrice"></Price>
				...
				>
					<div class="goods-list" :key="choosekey">
						...
						>
							...
							<div class="price">
								<!-- ${{ item.price }} -->
								<Price color="#E77B1B" size="14" :currency="item.price"></Price>
							</div>
							<div class="goods-img">
							...
							<div
								class="item-sell"
								v-if=" item.goodsType == 1 && item.statusType == 1 "
							>
								<div  @click="onExtractOne(item)">
									提取
								</div>
								<div @click="router.push( '/m/replacement' )">
									置换
								</div>
							</div>
							<div
								class="item-sell"
								v-if=" item.statusType == 5 "
							>
								<div>
									提取中
								</div>
								<div @click="CancelDraw(item)">
									取消提取
								</div>
							</div>
							<div
								class="item-sell"
								v-if="item.goodsType == 2"
							>
								<div @click="openRewardItem(item)">
									打开
								</div>
							</div>
						</div>
		...
		<div class="popup-content" :class="{active: open}">
			<div class="popup-wrap">
				<div class="close" @click="open = false">
					<Icon name="close" size="16" color="#fff"></Icon>
				</div>
				<span>本平台申明用户在本平台获得的只是游戏皮肤或道具不具备任何价值。通过用户在平台绑定的游戏交易链接赠送与用户的游戏账号里。本平台禁止一切回收，兑换，折现等方式。</span>
			</div>
		</div>
