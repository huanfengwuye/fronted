...
export default function axios(option) {
	return new Promise((resolve, reject) => {
		...
		instance.interceptors.request.use(config => {
			...
			// {
			// 	case 'zh' :
			// 		config.headers["X-Req-Lang-Key"] = 'zh_CN'
			// 	break

			// 	case 'tw' :
			// 		config.headers["X-Req-Lang-Key"] = 'zh_TW'
			// 	break
			// }
			return config
		}, err => {
			...
			response => {
				...
				if (response.status === 200)
				{
					if (response.data.code == 0) {
						...
						if( _isMobile() )
							window.NotifyF(response.data.message);
						else
							window.Warn( response.data.message )
					...
					//token 失效
					if( _isMobile() )
						window.NotifyF( t( 'network.unauthorized' ) )
					else
						window.Warn(  t( 'network.unauthorized' )  )
			...
			error => {
				...
				if (error && error.response)
				{
					...
					if (error.response.data && error.response.data.message) {
						if( _isMobile() )
							window.NotifyF(error.response.data.message);
						else
							window.Warn( error.response.data.message )
					} else {
						if( _isMobile() )
							window.NotifyF( t( 'network.error' ) )
						else
							window.Warn(  t( 'network.error' )  )
					}
					let res = {};
					res.code = 100;
					...
					if(error&&error.code=="ECONNABORTED"){
						if( _isMobile() )
							window.NotifyF( t( 'network.overtime' ) )
						else
							window.Warn(  t( 'network.overtime' )  )
					}else{
						if( _isMobile() )
							window.NotifyF( t( 'network.error' ) )
						else
