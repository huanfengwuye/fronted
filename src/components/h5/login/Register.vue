...
function initInviteCode() {
	...
	{
		telReg.value.inviteCode = localStorage.getItem( 'mskins_fcode' )
		mailReg.value.inviteCode = localStorage.getItem( 'mskins_fcode' )
...
async function onPhoneReg(v) {
	...
	let params = {};
// 使用正则表达式确保手机号码是11位数字
	params.mobile = /^\d{11}$/.test(v.tel) ? v.tel : `0${v.tel}`;
