...
<template>
			...
			<div class="act-box-list" v-if="false">
				...
				>
					...
					>
						...
						<Price :currency="value.price" color="#fff"></Price>
						</div>
					</lazy-component>
					</van-swipe-item>
				</van-swipe>
	...
	<!-- <div class="new-active">
		<div class="active-btn" @click="$router.push( '/m/newyear' )"></div>
	</div> -->

	<div class="home-list" v-for="(item, index) in homoBox" :key="index">
		<div
			class="title_bg"
			:class="{ title_hide: !item.show }"
		...
		<transition name="fade">
			...
			>
				...
				<lazy-component>
				<div class="new" v-show="value.labelUrl">
					<img :src="value.labelUrl" alt="" />
				</div>
				...
				<div class="item-name">{{ value.name }}</div>
				<div class="item-price">
					<Price color="#fff" font-weight="400" :currency="value.price"></Price>
					<span class="ori-price" v-if="Number( value.price ) < Number( value.originalPrice )">{{ value.originalPrice }}</span>
				</div>
				</lazy-component>
			</div>
			</div>
...
#home {
	...
	.new-active{
		...
		width: 730px;
		height: 700px;
		background: url("@/assets/romimg/home/newActive.png");
