...
import { nextTick, onMounted, ref, computed, watch, onBeforeMount } from "vue";
...
const showRechargeView = ref(false)//computed(() => store.state.showRechargeView); //ref( false )

/// 提示窗状态
const tipShow = computed( () => store.state.showRechargeView && hasReal.value )
...
const banks = [
	new URL("@/assets/pcimg/recharge/ABC.png", import.meta.url).href,
	new URL("@/assets/pcimg/recharge/mbBank.png", import.meta.url).href,
	new URL("@/assets/pcimg/recharge/techcomBank.png", import.meta.url).href,
	new URL("@/assets/pcimg/recharge/BIDV.png", import.meta.url).href,
];

const payTypeIcon = [
	new URL("@/assets/romimg/recharge/pay_money.png", import.meta.url).href,
	new URL("@/assets/romimg/recharge/ic_alipay.png", import.meta.url).href,
	new URL("@/assets/romimg/recharge/wxpay.png", import.meta.url).href,
	new URL("@/assets/romimg/recharge/icon_taobao.png", import.meta.url).href,
	...
	new URL("@/assets/pcimg/recharge/shande.png", import.meta.url).href,
	new URL("@/assets/pcimg/recharge/chang.png", import.meta.url).href,
];
...
///	是否已经实名认证
const hasReal = computed( () => store.getters.hasLogin && 
	userInfoExt.value.idNumber != '' && 
	userInfoExt.value.realname != '' 
)

watch( () => store.state.showRechargeView, val => {
	if( val && !hasReal.value )
		store.commit( 'setRealnameView', true )
	// console.log( val )
} )

onBeforeMount(() => {
	// window.$dev && console.log( hasReal.value )
	// if( !hasReal.value )
	// 	store.commit( 'setRealnameView', true )
})
...
async function getRechargeType() {
	...
	if (res.code == 0) { 
		...
		window.$dev && console.log("items",items);
	...
	if (res.code == 0) {
		...
		if (payUrl) {
			if (_isMobile()) {
				...
				// }
				// window.$dev && window.$dev && console.log( curRechageType.value )
				if( curRechageType.value.payType == 13 || curRechageType.value.merType == 34 ){
...
function onClickService() {
	window.$dev && console.log("11123123123")
  navShow = false;
  userShow = false;
  if (localStorage.getItem(tokenName)) {
    const channelId = store.getters.getConfigItem("CustomerServiceUrl") || "A12fdC"; // "C5j3Lu";
				...
				<div class="pc-recharge-main">
					...
					<div class="recg-title">
						...
						<!-- <div class="sub-title">
							...
							<svg @click="onClickCopyId" class="icon-c">
								<use xlink:href="/src/assets/fonts/icon.svg#copy"></use>
							</svg>
						</div> -->
					</div>
					<div class="recharge-wrap">
						<div class="type-wrap">
							...
							<div class="recharge-type-list">
								<div
									...
									:class="{ active: curRechageIndex == index }"
								>
									<!-- <div class="quan">
						...
						<div class="banks-sum" v-if="curRechageType.payType == 8">
							...
							<div class="banss">
								<div 
									...
									:key="index"
									@click="setbankType(index)"
									:class="{ active: bankLeb == index }"
								>
						...
						<div class="card-warp" v-if="curRechageType.payType == 12">
							...
							<div class="card-body">
								<div
									v-for="(item, index) in selectedCrads"
									:key="index"
									@click="setCardType(item,index)"
									class="cards"
						...
						<div class="amount-wrap">
							<div v-if="curRechageType.payType != 11" class="amount-tile">{{ t( 'users.rechargeMoney' ) }}</div>
							<div v-else class="amount-tile">
								<!-- {{ t( 'users.steamLink' ) }} -->
								<a href="https://steamcommunity.com/login/home/?goto=%2Fid%2Fme%2Ftradeoffers%2Fprivacy#trade_offer_access_url">
								{{ t( 'users.steamLink' ) }}</a>
								</div>
							<div class="recharge-list-wrap">
								<div
									class="card-pay-container"
									v-if="curRechageType.payType == 6"
								>
									...
									<div class="card-pay-form">
										<div class="card-pay-input">
											...
											<div class="pay-input">
												<input
													v-model="inputSteamContent"
													:placeholder="t( 'recharge.enterCardNoAndPassword' )"
													type="text"
													@change="clear()"
												/>
												<button
									...
									<div class="input-wrap">
										...
										<div class="btn-sure" @click="onBindSteam">
											<svg class="icon-check">
												<use xlink:href="@/assets/fonts/icon.svg#check"></use>
											</svg>
										</div>
									</div>
								</div>
								...
								<div class="recharge-warp" v-if="curRechageType.payType != 11"> 
									<div
										class="recharge-item"
										v-for="(item, index) in curRechageType.items"
										:key="index"
										@click="onClickMoneyItem(item, index)"
										:class="[
											{
						...
						<div class="card-warp" v-if="curRechageType.payType == 12">
							...
							<div class="card-body">
								<div class="card-info">
									...
									<input 
										type="text"
										:placeholder="t('recharge.serialNumberCard')"
										v-model="cardCode"
									>
								</div>
								<div class="card-info">
									...
									<input 
										type="text"
										:placeholder="t('recharge.codeUnderSilverLayer')"
										v-model="cardSecrets"
									>
								</div>
							</div>
							...
							<div class="reg-all">
								...
								<span>
									...
									<!-- <img
										...
										style="width: 24px"
									/> -->
									<!-- <Icon name="goldj" color="#FFB97A" size="19" ></Icon>  -->
									<!-- {{
									...
									<Price
										size="21" 
										:value="(Number(curMoneyItem.amount) +
										Number(curMoneyItem.giveAmount) )*
										(1-cardRate).toFixed(2)"
									>
									</Price>
					...
					<div class="pc-pop-code">
						<div class="pc-pop-code-bg">
							...
							<div
								...
								v-show="
									curRechageType.payType == 4 || curRechageType.payType == 5
								"
							>
								<div v-html="t( 'recharge.payMethod', { method : getRechargeTypeTitle(curRechageType.payType) } )"></div>
								<div>{{ t( 'recharge.scanTip' ) }}</div>
								<div class="userid" v-html="t( 'recharge.currentInfo', { userId : store.state.userId, area : 681 } )"></div>
								<div class="more-tip">{{ t( 'recharge.tipText' ) }}</div>
							</div>
							<div class="info-wrap" v-if="urlTpe == 1">
								<div class="info-wrap-left">
									<img src="@/assets/pcimg/recharge/hint.png" alt="">
								</div>
								<div class="info-wrap-right">
									<p>您尚未开户，请使用微信扫描二维码前往开户。</p>
									<span>本次充值使用账户余额，请开户后进行储值操作。</span>
								</div>
							</div>
							<div class="info-wrap" v-if="urlTpe == 2">
								<div class="info-wrap-left">
									<img src="@/assets/pcimg/recharge/hint.png" alt="">
								</div>
								<div class="info-wrap-right">
									<p>您的杉德账户余额不足，请使用微信扫描二维码前往充值。</p>
									<span>本次充值使用账户余额，请开户后进行储值操作。</span>
								</div>
							</div>
							<div
							...
							<div class="qr-hint">
								<div class="qr-completed" @click="refreshPage">
									我已完成充值
								</div>
								<div class="qr-question" @click="onClickService">
									充值遇到问题？
								</div>
							</div>
					...
					.card-warp{
						...
						.card-text{
							...
							.card-info{
								...
								input::placeholder {
									color: #969494; /* 更改占位符文本颜色 */
									font-size: 15px; /* 更改占位符文本字体大小 */
									/* 添加其他样式，如字体家族等，根据需要 */
								}
							}
							...
							.recharge-warp{
								...
								.recharge-item {
									...
									.quan {
										...
										align-items: center;
										justify-content: center;
										background: url(@/assets/pcimg/recharge/quan.png) center
											no-repeat;
