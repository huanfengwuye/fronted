...
async function onBindPhone(v) {
	...
	if (res.code === 0) {
		Success( { offset : 100, message : t( 'sign.bindPhoneSuccess' ) } )
...
<template>
	<div id="pc-bind-phone-wrap">
		<van-overlay :show="showBindPhoneiew" style="background-color: rgba(0,0,0,.8);">
			<div class="wrapper">
				<div class="signup-main">
					<div class="sign-tab-wrap">
						<div class="sign-tab-item active">
							{{ t( 'sign.bindPhoneTitle' ) }}</div>
					</div>
					<van-form @submit="onBindPhone" class="tel">
						<van-field v-model="telSign.tel" name="tel" label-width="0" 
							:placeholder="t( 'sign.enterPhoneNumber' )"
							:rules="[{ validator: validatorTel, message: t( 'sign.formatFail' ) }]" />
						<van-field v-model="telSign.verifyCode" center clearable name="verifyCode" label-width="0"
							...
							<template #button>
								<div class="btn-verification" v-show="countDownTel"
									@click="toCountDown(telSign.tel, 'countDownTel')">{{ countDownTelText }}</div>

								<van-count-down v-show="!countDownTel" ref="countDownVal" millisecond :time="60 * 1000"
									:auto-start="false" format="ss" @finish="countdownfinish('countDownTel')" />
							</template>
						</van-field>

						<div class="button_out">
							<van-button class="top" round block type="info" native-type="submit">{{ t( 'sign.bindConfirmBtn' ) }}</van-button>
						</div>
					</van-form>
				</div>
			</div>
