			...
			<div class="pw-packet-main" :class="{ 'active': Flip == 'just' }">
				<div class="pw-red-left">
					<van-notice-bar :scrollable="false" :delay="3" v-if="notice_show">
						<van-swipe vertical class="notice-swipe" :autoplay="3000" :show-indicators="false">
							<van-swipe-item v-for="(item, index) in recordList" :key="index">
								<span>{{ t( 'redpack.rewardText', { nickname : item.nickname || '', money : item.amount || 0 } ) }}</span>
							</van-swipe-item>
						</van-swipe>
					</van-notice-bar>
					<div class="red-list-wrap" v-if="allRed.length > 0">
						<div class="red-item-wrap" v-for="(item, index) in allRed" :key="index">
							...
							<div class="describe" v-html="t( 'redpack.receiveRule', { money : item.title } )"></div>
							<div class="red-item-time">
								<span>{{ t( 'redpack.activityTime' ) }}：{{ subtime(item.startTime) }} — {{ subtime(item.endTime) }}</span>
							</div>
							<div class="red-item-bottom">
								...
								<div class="red-item-btn" v-show="!item.isGet" @click="openRed(item.id)">{{ t( 'redpack.receive' ) }}</div>
				...
				<div class="red-content">
					...
					<div class="bottom-wrap">
						<input type="text" v-model="redKey" maxlength="16"
